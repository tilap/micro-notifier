...
try {
  ...
  router.use(async (ctx, next) => {
    ...
    const serviceFactory = new ServiceFactory({ aclCheck: false });
    if (token) {
      let user = await serviceFactory.get('accesstokens').getUserFromToken(token);
      if (user) {
        ctx.setState('user', user);
        ctx.apiResponse.setMeta('user', ctx.getState('user'));
        ctx.apiResponse.setMeta('rights', ctx.getService('users').getContextUserRights());
      }
    }
    await next();
    if (ctx.hasState('user')) {
